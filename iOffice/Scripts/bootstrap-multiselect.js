!function (b) {
    ...
    "undefined" !== typeof ko && ko.bindingHandlers && !ko.bindingHandlers.multiselect && (ko.bindingHandlers.multiselect = {
        init: function (a, c, d, e, f) {
            d = d().selectedOptions; c = ko.utils.unwrapObservable(c()); b(a).multiselex(c); m(d) && (b(a).multiselex("select", ko.utils.unwrapObservable(d)), d.subscribe(function (c) { var d = [], e = []; n(c, function (a) { switch (a.status) { case "added": d.push(a.value); break; case "deleted": e.push(a.value) } }); 0 < d.length && b(a).multiselex("select", d); 0 < e.length && b(a).multiselex("deselect", e) }, null,
            "arrayChange"))
        }, update: function (a, c, d, e, f) { d = d().options; e = b(a).data("multiselect"); c = ko.utils.unwrapObservable(c()); m(d) && d.subscribe(function (c) { b(a).multiselex("rebuild") }); e ? e.updateOriginalOptions() : b(a).multiselex(c) }
    }); k.prototype = {
        defaults: {
    ...
    }; b.fn.multiselect.Constructor = k; b(function () { b("select[data-role=multiselect]").multiselex() })
}(window.jQuery);
